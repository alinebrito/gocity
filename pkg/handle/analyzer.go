//go:generate go run github.com/markbates/pkger/cmd/pkger -o pkg/handle
...
func (h *AnalyzerHandle) Serve(port int) error {
	...
	file, err := pkger.Open("/pkg/handle/assets")
	if err != nil {
		return err
	}
	fs := http.FileServer(file)
